...
class Annotator.Plugin.RichText extends Annotator.Plugin
  ...
  pluginInit: ->
    ...
    annotator.subscribe "annotationEditorShown", ->
      $(annotator.editor.element).find(".tox-tinymce")[0].style.display = "block"
      $(annotator.editor.element).find(".tox-editor-container").css "z-index", 3000000000
      annotator.editor.checkOrientation()

    annotator.subscribe "annotationEditorHidden", ->
      $(annotator.editor.element).find(".tox-tinymce")[0].style.display = "none"
